...
test("owner primary tabs are one tap away on narrow screens and retain the desktop sidebar", async ({ page }) => {
  ...
  await expect(nav.getByRole("button")).toHaveText(["Today", "Menu", "Orders", "More"]);
  await expect(page.getByRole("button", { name: "Open owner menu" })).toHaveCount(0);
  for (const [tab, title] of [["Menu", "Menu"], ["Orders", "Orders"], ["Today", "Today"]]) {
...
test("More keeps secondary tools reachable and correctly marks their active group", async ({ page }) => {
  ...
  for (const destination of ["Settings", "Themes", "Analytics", "Staff", "QR Tables", "QR Inventory", "About Us"]) {
    ...
    await expect(primaryNav(page).getByRole("button", { name: "More" })).toHaveAttribute("aria-current", "page");
    await expect(primaryNav(page).getByRole("button", { name: "Orders" })).not.toHaveAttribute("aria-current", "page");
  }
  await primaryNav(page).getByRole("button", { name: "More" }).click();
...
test("More traps focus, dismisses without navigating, and handles native Back before app exit", async ({ page }) => {
  ...
  await expect(page.locator(".owner-header strong")).toHaveText("Today");
  await more.click();
...
test("offline tab navigation keeps the saved session and queued order IDs", async ({ page, context }) => {
  ...
  for (const tab of ["Orders", "Today", "Menu"]) {
